using DonVo2020.DataStrutures.NetCore31.Helpers;
using DonVo2020.DataStrutures.NetCore31.Trees.BinarySearchTree;
using System;
using System.Collections.Generic;
using System.Linq;

namespace DonVo2020.DataStrutures.NetCore31.Trees
{
    public static class TreeDrawer
    {
        ...
        /// </summary>
        public static string DrawTreeBTS<T>(this IBinarySearchTree<T> tree) where T : IComparable<T>
        {
            int position, width;
            return String.Join("\n", _recursivelyDrawTreeBTS(tree.Root, out position, out width));
        }
        public static string DrawTreeAVL<T>(this IBinarySearchTree<T> tree) where T : IComparable<T>
        {
            int position, width;
            return String.Join("\n", _recursivelyDrawTreeAVL(tree.Root, out position, out width));
        }
        public static string DrawTreeRBT<T>(this IBinarySearchTree<T> tree) where T : IComparable<T>
        {
            int position, width;
            return String.Join("\n", _recursivelyDrawTreeAVL(tree.Root, out position, out width));
        }

        public static string DrawTree<TKey, TValue>(IBinarySearchTree<TKey, TValue> tree, bool includeValues = false) where TKey : IComparable<TKey>
        {
            int position, width;
        ...
        // For RedBackTree
        private static List<string> _recursivelyDrawTreeRBT<T>
           (BSTNode<T> node, out int positionOutput, out int widthOutput)
           where T : IComparable<T>
        {
            widthOutput = 0;
            ...
            // Visit the left child
            List<string> leftLines = _recursivelyDrawTreeRBT(node.LeftChild, out leftPosition, out leftWidth);

            // Visit the right child
            List<string> rightLines = _recursivelyDrawTreeRBT(node.RightChild, out rightPosition, out rightWidth);

            // Calculate pads
            int middle = Math.Max(Math.Max(2, nodeLabel.Length), (rightPosition + leftWidth - leftPosition + 1)) + 5;
            int position_out = (leftPosition + middle / 2) + 5;
            int width_out = (leftPosition + middle + rightWidth - rightPosition) + 5;
        ...
        // for BST
        private static List<string> _recursivelyDrawTreeBTS<T>
           (BSTNode<T> node, out int positionOutput, out int widthOutput)
           where T : IComparable<T>
        {
            widthOutput = 0;
            ...
            // Visit the left child
            List<string> leftLines = _recursivelyDrawTreeBTS(node.LeftChild, out leftPosition, out leftWidth);

            // Visit the right child
            List<string> rightLines = _recursivelyDrawTreeBTS(node.RightChild, out rightPosition, out rightWidth);

            // Calculate pads
            int middle = Math.Max(Math.Max(2, nodeLabel.Length), (rightPosition + leftWidth - leftPosition + 1)) + 3;
            int position_out = (leftPosition + middle / 2) + 3;
            int width_out = (leftPosition + middle + rightWidth - rightPosition) + 3;
            ...
            where TKey : IComparable<TKey>
        {
            ...
            // Calculate pads
            middle = Math.Max(Math.Max(padValue, nodeLabel.Length), (rightPosition + leftWidth - leftPosition + 1)) + 3;
            position_out = leftPosition + middle + 3;
            width_out = leftPosition + middle + rightWidth - rightPosition + 3;
